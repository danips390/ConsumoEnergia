# -*- coding: utf-8 -*-
"""CódigoTomaDatos_V9_0.ipynb

Automatically generated by Colab.
ESTOY HACIENDO UN EJEMPLO
Original file is located at
    https://colab.research.google.com/drive/1-1CoSZw-sljA9oPbOTU4c6tVCDxlCBjz
"""
...
# CREDENCIALES
# -----------------------------------------------------------------------------
os.environ["TUYA_ACCESS_ID"] = "am4cw5rvxhehp848t9ys"
os.environ["TUYA_ACCESS_KEY"] = "cef3eb886ff940369f06785c01d998bd"
...
os.makedirs(DATA_CONV_FOLDER, exist_ok=True)
...
VIVIENDAS = {
    ...
    "JIH-V7": {
        ...
        "JIH-V18": {"tipo": "V3_DUAL", "device_id": "eb835b102903be7ef4cb1z"},
    },
    "JIH-V3A": {
    ...
    "JIH-V11": {
        "JIH-V11": {"tipo": "V3_DUAL", "device_id": "ebdeb7aab5f10913e6ufbe"},
    },
    "JIH-V19": {
        "JIH-V19": {"tipo": "V3_DUAL", "device_id": "ebc3f35aa1bd3211fbbyah"},
    },
    "JIH-V13": {
...
UNIFICACIONES = {
    ...
    "JIH-V11": {
        "archivo_salida": "JIH-V11.csv",
        "fase_A": "JIH-V11A",
        "fase_B": "JIH-V11B",
        "latitud": 25.660385069508177,
        "longitud": -100.28172812411054,
    },
    "JIH-V19": {
        "archivo_salida": "JIH-V19.csv",
        "fase_A": "JIH-V19A",
        "fase_B": "JIH-V19B",
        "latitud": 25.649651878648616,
        "longitud": -100.29444604007143,
    },
    "JIH-V13": {
        "archivo_salida": "JIH-V13.csv",
        "fase_A": "JIH-V13A",
        "fase_B": "JIH-V13B",
...
def obtener_datos_V3_DUAL(device_id, ts, prefijo="Medidor"):
    """
    Obtiene datos de un dispositivo V3_DUAL.

    Escalas corregidas según lectura in situ:
    - voltage_a / voltage_b: décimas de voltio  -> /10
    - current_a / current_b: miliamperios       -> /1000
    - power_a / power_b: décimas de watt        -> /10
    - power_factor / power_factor_b: porcentaje -> /100
    - freq: centésimas de Hz                    -> /100
    - energy_forword_a/b: centésimas de kWh     -> *10 para Wh
      Ejemplo:
        5671 -> 56.71 kWh -> 56710 Wh
    """
    ...
    # Fase A
    # -----------------------------
    voltage_a_V = propiedades.get("voltage_a", 0) / 10
    current_a_A = propiedades.get("current_a", 0) / 1000
    power_a_W = propiedades.get("power_a", 0) / 10
    pf_a = propiedades.get("power_factor", 0) / 100
    freq_Hz = propiedades.get("freq", 0) / 100
    energy_a_Wh = propiedades.get("energy_forword_a", 0) * 10

    # -----------------------------
    # Fase B
    # -----------------------------
    voltage_b_V = propiedades.get("voltage_b", 0) / 10
    current_b_A = propiedades.get("current_b", 0) / 1000
    power_b_W = propiedades.get("power_b", 0) / 10
    pf_b = propiedades.get("power_factor_b", 0) / 100
    energy_b_Wh = propiedades.get("energy_forword_b", 0) * 10

    datos_conv_split = {
        f"{prefijo}A": {
            "timestamp": ts,
            "voltage_V": voltage_a_V,
            "current_A": current_a_A,
            "power_W": power_a_W,
            "pf": pf_a,
            "freq_Hz": freq_Hz,
            "energy_Wh": energy_a_Wh,
            "energy_kWh": energy_a_Wh / 1000,
        },
        f"{prefijo}B": {
            "timestamp": ts,
            "voltage_V": voltage_b_V,
            "current_A": current_b_A,
            "power_W": power_b_W,
            "pf": pf_b,
            "freq_Hz": freq_Hz,
            "energy_Wh": energy_b_Wh,
            "energy_kWh": energy_b_Wh / 1000,
        },
    }
...
def obtener_datos_V3(device_id, ts):
    ...
    Escalas corregidas:
    - voltage_a: décimas de V       -> /10
    - current_a: mA                 -> /1000
    - power_a: décimas de W         -> /10
    - power_factor: porcentaje      -> /100
    - freq: centésimas de Hz        -> /100
    - energy_forword_a: 0.01 kWh    -> *10 para Wh
    """
    ...
    energy_Wh = propiedades.get("energy_forword_a", 0) * 10
...
def guardar_csv_unificado(base_folder, vivienda, archivo_salida, fila: dict):
    ...
    Si el archivo ya existe y aparecen nuevas columnas,
    conserva el histórico anterior, agrega las nuevas columnas
    y luego añade la nueva fila.
    """
    ...
    if not os.path.exists(archivo):
        cols = list(fila_formateada.keys())
    ...
    # Se conserva histórico y se agregan nuevas columnas si aparecen.
    # ---------------------------------------------------------
    try:
        df_existente = pd.read_csv(archivo, sep=";", dtype=str, on_bad_lines="skip")

        cols_existentes = list(df_existente.columns)
        cols_nuevas = list(fila_formateada.keys())

        # Unión conservando orden:
        # primero columnas existentes, luego columnas nuevas
        cols_finales = cols_existentes.copy()

        for col in cols_nuevas:
            if col not in cols_finales:
                cols_finales.append(col)

        # Agregar columnas faltantes al histórico
        for col in cols_finales:
            if col not in df_existente.columns:
                df_existente[col] = ""

        # Crear fila nueva con todas las columnas finales
        fila_completa = {col: fila_formateada.get(col, "") for col in cols_finales}

        df_nueva = pd.DataFrame([fila_completa])

        df_final = pd.concat(
            [df_existente[cols_finales], df_nueva[cols_finales]],
            ignore_index=True
        )
        ...
        print(f"⚠️ Error actualizando {archivo_salida}. Se intentará reconstruir solo con la fila actual. Error: {e}")
...
def consumos_horarios_ultimo_dia_para_geojson(group):
    ...
    como propiedades tipo consume6_7_Wh, consume7_8_Wh, etc.
    ...
    df_h.loc[df_h["consumo_Wh"] < 0, "consumo_Wh"] = None
    df_h = df_h.dropna(subset=["consumo_Wh"]).copy()

    for _, row in df_h.iterrows():
        h_ini = int(row["timestamp"].hour)
        h_fin = (h_ini + 1) % 24

        nombre_prop = f"consume{h_ini}_{h_fin}_Wh"
        props_consumo[nombre_prop] = round(float(row["consumo_Wh"]), 2)

    props_consumo["consumo_ultimo_dia_Wh"] = round(
        float(df_h["consumo_Wh"].sum()),
        2
    )
...
def calcular_promedio_consumo_horario_disponible(path_csv):
    ...
    # Energía total acumulada por las fases disponibles
    cols_energy = [
        col for col in ["energy_Wh_A", "energy_Wh_B", "energy_Wh_C", "energy_Wh_D"]
        if col in df.columns
    ]
    ...
    # Promedio por hora del día en Wh
    df_prom = (
        df_h
        .groupby("hora_dia", as_index=False)
        .agg(
            consumo_promedio_Wh=("consumo_Wh", "mean"),
            n_observaciones=("consumo_Wh", "count")
        )
    )

    df_prom["hora"] = df_prom["hora_dia"].apply(
...
try:
    ...
    for viv, sensores in VIVIENDAS.items():
        ...
        for sensor_name, info in sensores.items():
            ...
            if tipo == "V3_DUAL":
                datos_raw, datos_conv, err = getter(
                    device_id,
                    ts,
                    prefijo=sensor_name
                )
            ...
            if tipo in ["V1EXTRA_DUAL", "V3_DUAL"]:
                ...
                for sub_sensor_name, sub_datos in datos_conv.items():
                    ...
                    print(
                        ...
                        f"E={float(sub_datos.get('energy_kWh', sub_datos['energy_Wh'] / 1000)):.2f} kWh | "
                        f"Dir={direction_txt}"
                    )
                ...
                if cfg_uni is not None:
                    if sensor_name == cfg_uni.get("fase_A"):
                        datos_unificacion_ronda["A"] = datos_conv
                    elif sensor_name == cfg_uni.get("fase_B"):
                        datos_unificacion_ronda["B"] = datos_conv
                    elif sensor_name == cfg_uni.get("fase_C"):
                        datos_unificacion_ronda["C"] = datos_conv
                    elif sensor_name == cfg_uni.get("fase_D"):
                        datos_unificacion_ronda["D"] = datos_conv
                ...
                if tipo == "M1":
                    ...
                    print(
                        ...
                        f"E={float(datos_conv.get('energy_kWh', datos_conv['energy_Wh'] / 1000)):.2f} kWh"
                    )
        ...
        if cfg_uni is not None:
            ...
            if fase_a is not None and fase_b is not None:
                ...
                print(
                    f"🧩 Archivo unificado actualizado: "
                    f"{cfg_uni['archivo_salida']}"
                )

            else:
                print(
                    "⚠️ No se pudo construir archivo unificado en esta ronda: "
                    "falta Fase A o Fase B."
                )
    ...
    print("\n✅ Ronda única finalizada correctamente.")
